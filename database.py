...
# conn = sqlite3.connect(':memory:') ** create a temp database, deleted after program ends **
...
# create a table
# or use c.execute("CREATE TABLE customers (product_name DATATYPE,product_id DATATYPE,catalog DATATYPE,quantity DATATYPE,price DATATYPE,stock_date DATATYPE)")
...
# c.fetchone()
# c.fetchmany(3)
# print(c.fetchall())  display all data in DB
...
c.execute("SELECT  * FROM products WHERE product_name LIKE '%Corn' OR product_name LIKE 'GV%'") # search ****

print(c.fetchall())

# Update records
c.execute("""UPDATE products SET quantity = 11
...
# Delete records
# c.execute("""DELETE from products WHERE product_id = 'P001' """)
# c.execute("SELECT  * FROM products" )
# print(c.fetchall())

# Ordering
c.execute("SELECT  * FROM products ORDER BY price DESC") # sort by price from high to low
items = c.fetchall()
for i in items:
    print(i[0]+"\t"+i[1]+"\t"+i[2]+ "\t", i[3], "\t", i[4], "\t" + i[5])

# Display query with product_id = P001
c.execute("SELECT * FROM products WHERE product_id = 'P001'")
c.fetchall()
'''
...
def query():
    ...
    print_i = ""
    for i in records:
        print_i += (i[0]+"\t"+i[1] + "\t"+i[2] + "\t" + str(i[3]) + "\t" + str(i[4]) + "\t" + i[5]) + "\n"
    query_label = Label(root, text=print_i)
